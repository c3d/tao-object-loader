// *****************************************************************************
// glc_worldto3dxml.cpp                                            Tao3D project
// *****************************************************************************
//
// File description:
//
...
void GLC_WorldTo3dxml::addImageTextureTo3dxml(const QImage& image, const QString& fileName)
{
	...
	if (NULL != m_p3dxmlArchive)
	{
		...
		if (success)
		{
			image.save(m_pCurrentZipFile, QFileInfo(fileName).suffix().toUtf8().constData());
			m_pCurrentZipFile->close();
			delete m_pCurrentZipFile;
			m_pCurrentZipFile= NULL;
		}
	...
	else
	{
		...
		if (success)
		{
			image.save(m_pCurrentFile, QFileInfo(fileName).suffix().toUtf8().constData());
			delete m_pCurrentFile;
			m_pCurrentFile= NULL;
		}
	}
